...
# Usage: ./run_multi_bakeoff.sh <prompt_version> <start_run> <end_run>
set -euo pipefail
cd "$(dirname "$0")"
...
for run in $(seq "$START_RUN" "$END_RUN"); do
  ...
  ./run_model_bakeoff.sh "$PROMPT_VERSION" "$run"
done

python3 summarize_runs.py --prompt-version "$PROMPT_VERSION" --runs "$START_RUN-$END_RUN"
echo "Multi-run done."
